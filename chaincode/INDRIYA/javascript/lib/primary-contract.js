...
class PrimaryContract extends Contract {
    ...
    // Match-making between patient and donor
    async match(ctx, args) {
        ...
        for (let i = 0; i < allPatients.length; i++) {
            let p = allPatients[i]
            if (p.Record.docType == 'donor' && p.Record.match == '' && p.Record.bloodgroup == bloodgroup && p.Record.organRequired == organRequired && p.Record.gender == gender) {
                matches.push(p)
            }
        }
        return matches
    }
    // Delete a patient or donor
    ...
    // Cross-update donor and patient after hospital staff select one candidate out of the matched candidates.
